...
const ExclusiveOffers = () => {
  return (
    ...
    />
    <button className='group flex items-center gap-2 font-medium cursor-pointer max-md:mt-12 px-4 py-2 rounded-md hover:bg-gray-100 transition'>
        View All Offers
        ...
        {exclusiveOffers.map((item)=>(
            <div key={item._id} className='group relative flex flex-col items-start justify-between gap-1 pt-12 md:pt-18 px-4 rounded-xl text-white bg-no-repeat bg-cover bg-center'  style={{backgroundImage:`url(${item.image}) `}}>
                <p className='px-3 py-1 absoulute top-4 left-4 text-xs bg-white text-gray-800 font-medium rounded-full'>{item.priceOff}% OFF</p>
